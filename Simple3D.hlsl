...
struct VS_OUT
{
	...
	float4 eyepos	:	POSITION;		//視点（カメラ位置）
...
VS_OUT VS(float4 pos : POSITION,float4 eyepos : POSITION, float4 uv : TEXCOORD, float4 normal : NORMAL)
{
	...
	outData.color = saturate(reverseVec);
#endif
	//まとめて出力
	return outData;
}
...
float4 PS(VS_OUT inData) : SV_Target
{
	...
	if (isTexture) {
		...
		// 鏡面反射色（なんかツルツルなやつ）
		specular = pow(dot(inData.color, inData.eyepos - inData.pos), diffuseColor) * 2.0 * lightDirection;
	}
	else {
		// 拡散反射色（なんか明るいやつ）
